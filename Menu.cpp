...
void Menu::Click(sf::RenderWindow& window, float width, float height) //This function is detecting when the user mouses want to start the game or exit the game
{
	sf::Vector2i postion = sf::Mouse::getPosition(window);		//This gets the position of the mouse
	level nextlevel(window.getSize().x, window.getSize().y);	//level constructor
	int complete = 0;
